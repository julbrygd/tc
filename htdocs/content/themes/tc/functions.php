...
$scripts = $theme['config.factory']->get('scripts');
$use_cdn = defined('USE_CDN') ? USE_CDN : false;
foreach ($scripts as $script_slug => $s) {
    if (!isset($s["deps"])) {
        $s["deps"] = [];
    }
    if ($use_cdn && isset($s["cdn"])) {
        $s["path"] = $s["cdn"];
    }
    if (!isset($s["version"])) {
        $s["version"] = "1.0";
    ...
    if (!isset($s["mixed"])) {
        if (isset($s["stylesheet"])) {
            $s["mixed"] = $s["stylesheet"];
        } else if (isset($s["footer"])) {
            $s["mixed"] = $s["footer"];
        } else {
            $s["mixed"] = null;
        }
    ...
    Asset::add(
            $script_slug, $s["path"], $s["deps"], $s["version"], $s["mixed"], $s["type"]
    );
}
...
function bootstarp_header_menu() {
    wp_nav_menu(
            array(
...
require_once themosis_path("theme") . ".." . DS . ".." . DS . "plugins" . DS . "wp-bootstrap-navwalker" . DS . "wp-bootstrap-navwalker.php";
